...
- [ ] Module 1: Executive Dashboard (KPIs, graphs, widgets)
...
7. **This project folder is not its own git repo** — `git rev-parse --show-toplevel` resolves to the whole home directory (`C:\Users\DELL-G3 15-3500`), so `server/` and `admin/` show up as untracked inside a repo that spans the entire home folder. Likely an accidental `git init` from home at some point. Not yet fixed — avoid `git add -A`/`git add .` from the home directory until this is scoped properly.
...
5. **Git repo scoping** — see note above, not yet addressed. Getting more overdue as the codebase grows.
6. **Storefront (`html/`) has no connection to the database at all** — admin panel changes (products, orders, blog posts, customers, etc.) do not show up on the customer-facing site, which is still 100% static HTML from the original template. This is also why CMS's homepage/banner/menu pieces were skipped — no consumer to wire them to yet. Not scoped or started; flagged to the user 2026-07-21.
