...
- **🔴 Real checkout was broken — fixed, in two rounds**: `server/routes/public.js`'s checkout (`POST /api/public/orders`) turned out to have **three** places where it inserted into columns/RPCs that no migration had ever actually created — the whole handler had apparently never been tested against the real deployed schema (testing had only gone through the admin's own order-creation dialog, a different code path). Found by actually clicking "Place Order" and reading each schema-cache error in turn, fixing, retrying:
  ...
  4. `duplicate key value violates unique constraint "customers_email_idx"` — a real logic bug, not a schema gap (schema was already correct here). The customer-upsert step looked up "existing customer" by **phone only**, but `customers`' actual unique constraint (`phase4_customers.sql`) is **on email** — phone isn't unique-constrained at all. A phone-only `.maybeSingle()` lookup silently fails (returns no match, no error surfaced) if more than one row shares that phone number — which had already happened, from the customer-insert step succeeding on some of the earlier failed attempts above before the *order* insert then failed on #1/#2. The lookup missed the existing row and fell through to insert, colliding on email. Fixed by upserting directly against the real constraint (`.upsert(..., { onConflict: 'email' })`) instead of a separate select-then-insert — checkout.html requires email, so this is now the normal path; a phone-based best-effort lookup remains only as a fallback for the case where email is missing (the server itself doesn't require it, only the form does).
  #2 and #3 fixed together in `server/supabase/phase10_checkout_schema_gaps.sql`; #4 needed no migration (application logic only). Both new migrations registered in `server/scripts/migrate.js`. **This handler had 4 real bugs stacked on top of each other, found one at a time by actually clicking "Place Order" repeatedly — if it throws yet another error, keep going the same way rather than assuming it's fully clean now.**
