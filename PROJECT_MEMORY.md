...
- **🔴 Real checkout was broken — fixed**: placing an order failed with `Could not find the 'payment_method' column of 'orders' in the schema cache`. `server/routes/public.js`'s checkout (`POST /api/public/orders`) has always inserted `payment_method: 'cod'` / `payment_status: 'unpaid'` on every order, but **no migration ever actually added those columns** — `phase3_orders.sql` predates real checkout and doesn't have them, and nothing since added them either (this had presumably been silently broken for a while; not caught earlier because most testing happened through the admin's own order-creation dialog, not the public storefront checkout). New `server/supabase/phase9_order_payment_method.sql` adds both, registered in `server/scripts/migrate.js`.
...
### Current Status
- **🔴 Not yet run, actively blocking checkout**: `server/supabase/phase9_order_payment_method.sql` hasn't been applied to the live Supabase instance from this session (no DB credentials available there) — **the public storefront checkout is broken until this runs** (`Could not find the 'payment_method' column of 'orders' in the schema cache`). Run `npm run migrate` or paste it into the Supabase SQL Editor — this one first, it's a real outage, not a missing nice-to-have.
